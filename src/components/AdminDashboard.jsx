...
export default function AdminDashboard(props) {
    ...
    const extractPdfCover = async (file) => {
        try {
            ...
            const viewport = page.getViewport({ scale: 1.5 }); // Scale up for better quality cover
            ...
            return new Promise((resolve) => {
                canvas.toBlob((blob) => {
                    if (blob) {
                        const imgFile = new File([blob], "cover-extracted.jpg", { type: "image/jpeg" });
                        resolve(imgFile);
                    } else {
                        resolve(null);
                    }
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-black/98 overflow-hidden font-technical">
            ...
            <div className="w-full max-w-7xl bg-bg-black border border-zinc-900 shadow-[0_0_100px_rgba(0,0,0,1)] h-full md:h-[95vh] flex flex-col relative z-10 overflow-hidden">

                {/* Tactical Top Bar */}
                <div className="flex flex-col md:flex-row justify-between items-center px-6 py-4 border-b border-zinc-900 bg-black/50 backdrop-blur-md shrink-0 gap-4">
                    <div className="flex items-center gap-6">
                        <div className="relative">
                            <h2 className="text-xl md:text-2xl font-technical text-white uppercase tracking-[0.2em] font-bold">OPERATIONS_HUB</h2>
                            <div className="absolute -bottom-1 left-0 w-full h-[1px] bg-fire/50 animate-pulse-slow"></div>
                        </div>
                        <div className="flex items-center gap-2 px-3 py-1 bg-fire/10 border border-fire/20">
                            <div className="w-1.5 h-1.5 rounded-full bg-fire animate-pulse"></div>
                            <span className="text-[9px] font-technical text-fire uppercase font-bold">SECURE_CHANNEL_READY</span>
                        </div>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="text-right hidden md:block">
                            <p className="text-[8px] text-zinc-600 uppercase">LOGGED_IN_AS</p>
                            <p className="text-[10px] text-white uppercase tracking-wider">{currentUser?.email || 'OFFLINE_BYPASS'}</p>
                        </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    {/* Sidebar Tabs */}
                    <div className="w-16 md:w-56 border-r border-zinc-900 bg-black/40 flex flex-col overflow-y-auto custom-scrollbar">
                        {[
                            { id: 'BOOKS', icon: 'auto_stories', label: 'ARCHIVES' },
                            { id: 'DRILLS', icon: 'precision_manufacturing', label: 'PROTOCOLS' },
                            { id: 'USERS', icon: 'person_search', label: 'OPERATIVES' },
                            { id: 'SUBS', icon: 'credit_score', label: 'ACCESS_LEVELS' },
                            { id: 'ENLISTMENTS', icon: 'broadcast_on_personal', label: 'SIGNALS_RAW' },
                            { id: 'APPLICATIONS', icon: 'assignment_ind', label: 'RECRUITMENT' },
                            { id: 'INTENTS', icon: 'shopping_cart_checkout', label: 'INTEL_MARKET' },
                            { id: 'SIGNALS', icon: 'chat', label: 'COMMS' },
                            { id: 'CONFIG', icon: 'settings_input_component', label: 'PROTOCOL_CFG' },
                            { id: 'LOGS', icon: 'terminal', label: 'SYSTEM_LOGS' }
                        ].map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center gap-4 px-4 py-4 md:py-5 border-b border-zinc-900/50 transition-all group relative ${activeTab === tab.id ? 'bg-fire/5 text-fire' : 'text-zinc-600 hover:text-zinc-300 hover:bg-zinc-900/30'}`}
                            >
                                {activeTab === tab.id && <div className="absolute left-0 top-0 bottom-0 w-1 bg-fire shadow-[0_0_10px_rgba(255,77,0,0.5)]"></div>}
                                <span className={`material-symbols-outlined text-2xl ${activeTab === tab.id ? 'text-fire' : 'group-hover:text-fire/70 transition-colors'}`}>
                                    {tab.icon}
                                </span>
                                <span className="hidden md:block text-[10px] font-bold tracking-widest uppercase truncate">{tab.label}</span>
                            </button>
                        ))}
                        ...
                        <div className="mt-auto p-4 flex flex-col gap-2">
                            <button
                                ...
                                className="flex items-center gap-4 px-2 py-3 text-zinc-600 hover:text-fire transition-colors group"
                            >
                                <span className="material-symbols-outlined text-xl">logout</span>
                                <span className="hidden md:block text-[9px] font-bold uppercase tracking-tighter">TERMINATE_SESSION</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col bg-black/20 relative">
                        {/* Tab Background Decorative Element */}
                        <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
                            <span className="material-symbols-outlined text-[300px]">
                                {['BOOKS', 'DRILLS', 'USERS', 'SUBS', 'ENLISTMENTS', 'APPLICATIONS', 'INTENTS', 'SIGNALS', 'CONFIG', 'LOGS'].find(t => t === activeTab) ?
                                    {
                                        'BOOKS': 'auto_stories',
                                        'DRILLS': 'precision_manufacturing',
                                        'USERS': 'person_search',
                                        'SUBS': 'credit_score',
                                        'ENLISTMENTS': 'broadcast_on_personal',
                                        'APPLICATIONS': 'assignment_ind',
                                        'INTENTS': 'shopping_cart_checkout',
                                        'SIGNALS': 'chat',
                                            ...
                                            <div className="border border-zinc-700 p-3 flex flex-col gap-2 bg-zinc-900/20">
                                                ...
                                                <input
                                                    ...
                                                    onChange={async (e) => {
                                                        ...
                                                        if (file) {
                                                            setNewItem(prev => ({ ...prev, pdfFile: file }));
                                                            // Auto-extract cover if no image is currently selected
                                                            if (!newItem.imgFile) {
                                                                setUploadStatus("EXTRACTING_COVER_FROM_PDF...");
                                                                const cover = await extractPdfCover(file);
                                                                if (cover) {
                                                                    setNewItem(prev => ({ ...prev, pdfFile: file, imgFile: cover }));
                                                                    setUploadStatus(""); // Clear status
                                                                    alert("COVER_EXTRACTED_FROM_PDF_PROTOCOL");
                                                                } else {
                                                                    setUploadStatus("");
                                                                }
                                                            }
